function F=Mod_ReturnFn(n,f,aprime,a,zh,zn,e,theta_i,kappa_j,pen_j,medspend_j,...
...
F=-Inf;
% Now the preference shifter affects util consumpt but not labor disutil
if cons>0
    effort_disutil = iota*f^(1+1/psi)/(1+1/psi);
    F = (cons^(1-sigma))/(1-sigma)-shifter*nu*n^(1+1/gamma)/(1+1/gamma)-effort_disutil; 
end
